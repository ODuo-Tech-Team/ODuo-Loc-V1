...
export function ActivitiesClient({ initialData }: { initialData: InitialData }) {
  const [activities, setActivities] = useState<ActivityLog[]>(initialData.activities)
  const [tenants] = useState<Tenant[]>(initialData.tenants)
  const [stats, setStats] = useState<Record<string, number>>(initialData.stats)
  ...
  const fetchActivities = async (newPage?: number) => {
    try {
      setLoading(true)
  ...
  const handleFilterChange = (filter: string, value: string) => {
    if (filter === "tenant") setTenantFilter(value)
    if (filter === "action") setActionFilter(value)
    if (filter === "entity") setEntityFilter(value)
    setPage(1)
    // Delay para evitar múltiplas chamadas
    setTimeout(() => fetchActivities(1), 100)
  }
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Select value={tenantFilter} onValueChange={(v) => handleFilterChange("tenant", v)}>
              ...
              <Select value={entityFilter} onValueChange={(v) => handleFilterChange("entity", v)}>
                <SelectTrigger className="bg-white/5 border-white/10">
